...
/**
 * Novitus Online Fiscal Printer — Odoo 17
 *
 * ARCHITECTURE: JS sends order reference to Python backend.
 * Python does ALL payload construction and printer communication.
 * JS handles UI notifications and stores fiscal data on the order.
 *
 * RPC calls go through this.pos.orm.call() — the PosStore instance
...
export class NovitusPrinter extends BasePrinter {
    ...
    // ── Helper: RPC call to Python backend ────────────────────

    async _rpc(model, method, args) {
        return await this.pos.orm.call(model, method, args);
    }

    _notify(message, type = 'info', sticky = false) {
        this.pos.env.services.notification.add(message, { type, sticky });
    }
